...
const breakpointsToSizes = (breakpoints: MediaBreakpoints) => Object.keys(breakpoints).reduce(
    (acc, key) => ({
        ...acc,
        [key]: typeof window !== 'undefined' && window.innerWidth > breakpoints[key],
    }), {}
);
...
export function useMediaSizes(customBreakpoints?: MediaBreakpoints): MediaSizes {
    ...
    const [sizes, setSizes] = useState<MediaSizes>(() => typeof window !== 'undefined' && breakpointsToSizes(breakpoints));

    useEffect(() => {
        if (typeof window === 'undefined') return; // Return if window object is not available
